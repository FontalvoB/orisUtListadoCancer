...
import { importArthritisRecords } from '../services/arthritisService';
...
interface ImportArthritisExcelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImportComplete: () => void;
}

export default function ImportArthritisExcelModal({ isOpen, onClose, onImportComplete }: ImportArthritisExcelModalProps) {
  const { user } = useAuth();
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const imported = await importArthritisRecords(records, (completed, total) => {
          ...
          await logActivity({
            ...
            action: 'import',
            module: 'arthritis',
            description: `Importación Excel Artritis: ${imported.toLocaleString()} registros desde ${file.name}`,
    ...
    <div className="modal-overlay">
      <div className="modal modal-large">
        <div className="modal-header">
          <h2><HiDocumentText /> Importar Excel - Artritis</h2>
